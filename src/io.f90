      ...
      ENDDO
      WRITE(IO,'(A)') '#'
      WRITE(IO,'(A1(13X)AXA)') '#','DET. MECH.','RED. MECH.'
      WRITE(IO,'(A1XA11X2(I8X))') '#','NREAC=',NREAC,SUM(SETRE)
      WRITE(IO,'(A1XA11X2(I8X))') '#','NSPEC=',NSPEC,SUM(SETSP)
      WRITE(IO,'(A1XA11X2(I8X))') '#','NREAC-BOLS=',NREAC_BOLSIG, &
                                 NREAC_BOLSIG_SKEL
      WRITE(IO,'(A1XA11X2(I8X))') '#','NSPEC-BOLS=',NSPEC_BOLSIG, &
                                 SUM(SETSP_BOLS)
      ...
      USE CHEM_PARSE, ONLY : CM_GET_SPECIES_INDEX
      IMPLICIT NONE
      CHARACTER(LEN=72) :: BUILD_LINE
